...
class VolatilityContributor(SignalContributor):
    def compute(self, data: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        try:
            monday = _compute_risk_metrics(data.get('ts'), data.get('vix_data'), data.get('levels'), data.get('price'), data.get('atr_v'))
            return {'volatility': monday}
        except: return {}
